...
def launch_setup(context, *args, **kwargs):      
    ...
    return [
        ...
        Node(
            ...
            parameters=[{
                "frame_id": LaunchConfiguration('frame_id'), # Changed from frame_id
                "odom_frame_id": LaunchConfiguration('vo_frame_id'),
                "publish_tf": LaunchConfiguration('publish_tf_odom'),
                "ground_truth_frame_id": LaunchConfiguration('ground_truth_frame_id').perform(context),
                "ground_truth_base_frame_id": LaunchConfiguration('ground_truth_base_frame_id').perform(context),
            ...
            remappings=[
                ...
                ("odom", LaunchConfiguration('dog_odom')), # Changed from odom_topic
                ("imu", LaunchConfiguration('imu_topic'))],
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('frame_id',       default_value='base_link',          description='Fixed frame id of the robot (base frame), you may set "base_link" or "base_footprint" if they are published. For camera-only config, this could be "camera_link".'), # Changed to base_link from base_footprint
        ...
        DeclareLaunchArgument('qos',            default_value='2',                  description='General QoS used for sensor input data: 0=system default, 1=Reliable, 2=Best Effort.'), # Changed from 1 to 2.
        ...
        DeclareLaunchArgument('scan_cloud_topic',     default_value='/rslidar_points', description=''), # Changed to /rslidar_points from /velodyne_points/point
        ...
        DeclareLaunchArgument('odom_topic',                 default_value='dog_odom',  description='Odometry topic name.'),
